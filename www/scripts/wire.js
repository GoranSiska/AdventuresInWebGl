...
const main = function (canvas, image, configuration) {
    // viewport
    ...
    int getCellTypeByColor (vec4 cellColor) {
        ...
        if (cellColor.r == 1.0) {
            return 2;
        // is it tail
        } else if (cellColor.g == 1.0) {
            return 3;
        } else if (cellColor.b == 1.0) {
            return 1;
        }
        // empty
        return 0;        
    ...
    int getNeighborElectronsCount (vec2 coord) {
        ...
        if (coord.y > 1.0) {
            //left
            if (coord.x > 1.0) {
                if (getCellTypeByColor(getCellColor(vec2(coord.x - 1.0, coord.y - 1.0))) == 2) {
                    neighborElectronCount += 1;
                }
            }
            // center
            if (getCellTypeByColor(getCellColor(vec2(coord.x, coord.y - 1.0))) == 2) {
                neighborElectronCount += 1;
            }
            // right
            if (coord.x < ${width}.0)
        ...
        // middle
        if (coord.x > 1.0) {
            if (getCellTypeByColor(getCellColor(vec2(coord.x - 1.0, coord.y))) == 2) {
                neighborElectronCount += 1;
            }
        }
        // right
    ...
    void main() {
        ...
        if (electronNeighborCount == 1 || electronNeighborCount == 2) {
